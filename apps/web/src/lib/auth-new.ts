...
const customAdapter = {
  async createUser(user: any) {
    console.log('🔧 Custom adapter: createUser called with:', user.email)
    ...
    console.log('✅ Custom adapter: createUser result:', result[0]?.id)
    return result[0]
  },
  ...
  async getUser(id: string) {
    ...
    console.log('✅ Custom adapter: getUser result:', result[0]?.email)
  ...
  async getUserByEmail(email: string) {
    ...
    console.log('✅ Custom adapter: getUserByEmail result:', result[0]?.id)
    return result[0] || null
  },
  
  async getUserByAccount({ providerAccountId, provider }: any) {
    console.log('🔧 Custom adapter: getUserByAccount called with:', { providerAccountId, provider })
    const result = await directQuery(
      'SELECT u.* FROM "User" u JOIN "Account" a ON u.id = a."userId" WHERE a."providerAccountId" = $1 AND a.provider = $2',
      [providerAccountId, provider]
    )
    console.log('✅ Custom adapter: getUserByAccount result:', result[0]?.email)
    return result[0] || null
  },
  
  async updateUser(user: any) {
    console.log('🔧 Custom adapter: updateUser called with:', user.id)
    const result = await directQuery(
    ...
    console.log('✅ Custom adapter: updateUser result:', result[0]?.id)
    return result[0]
  },
  
  async deleteUser(userId: string) {
    console.log('🔧 Custom adapter: deleteUser called with:', userId)
  ...
  async linkAccount(account: any) {
    ...
    console.log('✅ Custom adapter: linkAccount result:', result[0]?.id)
    return result[0]
  },
  
  async unlinkAccount({ providerAccountId, provider }: any) {
    console.log('🔧 Custom adapter: unlinkAccount called with:', { providerAccountId, provider })
    await directQuery(
  ...
  async getSessionAndUser(sessionToken: string) {
    console.log('🔧 Custom adapter: getSessionAndUser called with:', sessionToken.substring(0, 10) + '...')
    const result = await directQuery(
      'SELECT s.*, u.* FROM "Session" s JOIN "User" u ON s."userId" = u.id WHERE s."sessionToken" = $1',
      [sessionToken]
    )
    if (!result[0]) {
      console.log('❌ Custom adapter: getSessionAndUser - no session found')
      return null
    }
    console.log('✅ Custom adapter: getSessionAndUser result:', result[0].email)
    return {
      session: {
        id: result[0].id,
  ...
  async deleteSession(sessionToken: string) {
    console.log('🔧 Custom adapter: deleteSession called')
  ...
  async useVerificationToken({ identifier, token }: any) {
    ...
    console.log('✅ Custom adapter: useVerificationToken result:', result[0]?.id)
    return result[0] || null
  }
}
