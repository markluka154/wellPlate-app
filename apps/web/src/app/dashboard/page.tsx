      ...
      const requestData = {
        preferences: {
          ...
          dietType: diet === 'Balanced' ? 'omnivore' :
                   ...
                   diet.toLowerCase(),
          allergies: allergies.split(',').map(a => a.trim()).filter(Boolean), // Allergies are free for all users
          dislikes: userPlan !== 'FREE' ? dislikes.split(',').map(d => d.trim()).filter(Boolean) : [], // Dislikes are Pro only
          ...
          <div className="relative overflow-hidden rounded-3xl border border-gradient-to-r from-emerald-200/50 to-blue-200/50 bg-gradient-to-br from-white via-emerald-50/30 to-blue-50/30 shadow-xl h-full flex flex-col">
            ...
            <div className="relative p-4 sm:p-6 lg:p-8">
              ...
              <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2">
                ...
                {/* Allergies - Free for all users */}
                <FormField label="Food Allergies" hint="Specify foods to avoid for personalized meal plans">
                  <input
                    type="text"
                    value={allergies}
                ...
                <ProBadge 
                  showBadge={userPlan === 'FREE'}
                  onClick={() => userPlan === 'FREE' && showUpgrade('Food Dislikes', 'Specify food dislikes for personalized meal plans with Pro.', 'Food dislikes')}
                >
                  <FormField label="Food Dislikes (Pro)" hint="Specify foods you don't like for personalized meal plans">
                    <input
                      type="text"
                      value={dislikes}
